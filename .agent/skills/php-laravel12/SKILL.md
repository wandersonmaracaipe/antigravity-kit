...
## 8) Fast Discovery Protocol (Token-Efficient)

When the request is about locating controllers/routes/models, do this first:

1. Run deterministic mapper (no trial-and-error regex loops):

```bash
python .agent/scripts/laravel_map.py --root <project_root>
```

2. If searching specific controller:

```bash
python .agent/scripts/laravel_map.py --root <project_root> --controller UserController
```

3. Then inspect target files directly (`app/Http/Controllers`, `routes/api.php`, `routes/web.php`).

**Avoid by default:**
- Long ad-hoc PowerShell scripts for framework discovery
- Regex-heavy shell loops just to detect Laravel structure
- Repeated failed commands with no new signal
